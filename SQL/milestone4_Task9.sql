...
DiffTimeOrder AS (
                  SELECT order_time, next_order_time, DATE_PART('year', order_time) as year, 
                    DATE_PART('hour', next_order_time::timestamp - order_time::timestamp) as dhour,
...
AvgTime AS (
              SELECT year, EXTRACT(HOUR FROM dstamp::interval) as hour, EXTRACT(minute FROM dstamp::interval) as minute, 
                EXTRACT(second FROM dstamp::interval) as seconds, EXTRACT(milliseconds FROM dstamp::interval) as milliseconds
                FROM AvgOrder
